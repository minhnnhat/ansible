if [ -d "/home/minhnn41" ]
then
        echo "minhnn41 exists"
else
        useradd minhnn41
        mkdir /home/minhnn41/.ssh
        chown -R minhnn41: /home/minhnn41/.ssh
        cd /home/minhnn41/.ssh
        touch authorized_keys
        echo "ssh-rsa  nhatminh@NhatMinh" > authorized_keys
        chown -R minhnn41: /home/minhnn41/.ssh/authorized_keys
        chmod 700 /home/minhnn41/.ssh
        chmod 600 /home/minhnn41/.ssh/authorized_keys
        echo "minhnn41        ALL=(ALL)       NOPASSWD: ALL" >> /etc/sudoers
        echo "done minhnn41"
fi
